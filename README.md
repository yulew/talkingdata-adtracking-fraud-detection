...
## XGBoost
A random forest or a gradient boosting is a set of decision trees. XGBoost is an implementation of gradient boosted decision trees that are dominatedly applied in Kaggle competitions due to its good speed and performance. A decision tree relies on keeping finding a splitting point on each feature but can cause overfitting. A random forest or a gradient boosting can reduce the overfitting by introducing many decision trees.
